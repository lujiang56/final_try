"""科目管理 / 仪表盘 / 考点 / 突击计划 / 错题管理"""

from flask import Blueprint, render_template, request, redirect, url_for, jsonify
from database import (
    get_all_exams, get_exam, create_exam, update_exam, delete_exam,
    get_sections, update_section,
    get_knowledge_points, toggle_kp_mastered, delete_kp,
...
@exam_bp.route('/exam/<int:exam_id>', methods=['GET', 'POST'])
def subject(exam_id):
        ...
        for i, sid in enumerate(section_ids):
            if i < len(section_scores):
                update_section(int(sid),
                               float(section_scores[i] or 0),
                               int(section_counts[i] or 0))
        return redirect(url_for('exam.subject', exam_id=exam_id))
...
@exam_bp.route('/exam/<int:exam_id>/materials', methods=['GET', 'POST'])
...
# ─── PPT 上传（传统表单流 → 跳转到新版 PPT 分析页面） ───

@exam_bp.route('/exam/<int:exam_id>/materials/upload', methods=['POST'])
def upload_ppt(exam_id):
    ...
    if 'ppt_file' not in request.files:
        return redirect(url_for('exam.materials', exam_id=exam_id))
    ...
    if not allowed_file(file.filename):
        return '不支持的文件格式，请上传 .pptx 文件', 400
    ...
    return redirect(url_for('ppt.ppt_analysis_session', session_key=session_key))


# ─── 复习资料生成（传统表单流） ─────────────────────────

@exam_bp.route('/exam/<int:exam_id>/materials/generate', methods=['POST'])
def generate_material_route(exam_id):
    exam = get_exam(exam_id)
    if not exam:
        return redirect(url_for('exam.dashboard'))

    material_type = request.form.get('material_type', 'summary')
    from material_generator import generate_material as gen_mat
    gen_mat(exam_id, material_type=material_type, is_from_ppt=False)
...
@exam_bp.route('/exam/<int:exam_id>/add_kp', methods=['POST'])
def add_kp(exam_id):
    ...
    return redirect(request.referrer or url_for('exam.subject', exam_id=exam_id))


# ─── 切换考点掌握状态 ─────────────────────────────────────

@exam_bp.route('/exam/<int:exam_id>/kp/<int:kp_id>/toggle', methods=['POST'])
...
@exam_bp.route('/exam/<int:exam_id>/kp/<int:kp_id>/delete', methods=['POST'])
...
@exam_bp.route('/exam/<int:exam_id>/plan')
def plan(exam_id):
    exam = get_exam(exam_id)
    if not exam:
        return redirect(url_for('exam.dashboard'))
...
@exam_bp.route('/exam/<int:exam_id>/plan/regenerate', methods=['POST'])
def plan_regenerate(exam_id):
    generate_plan(exam_id)
    return redirect(url_for('exam.plan', exam_id=exam_id))


@exam_bp.route('/exam/<int:exam_id>/plan/toggle/<int:task_id>', methods=['POST'])
...
@exam_bp.route('/exam/<int:exam_id>/mistake', methods=['POST'])
...
@exam_bp.route('/exam/<int:exam_id>/mistake/<int:mid>/toggle', methods=['POST'])
def mistake_toggle(exam_id, mid):
    toggle_mistake_reviewed(mid)
    return redirect(request.referrer or url_for('exam.subject', exam_id=exam_id))


@exam_bp.route('/exam/<int:exam_id>/mistake/<int:mid>/delete', methods=['POST'])
...
@exam_bp.route('/api/exam/<int:exam_id>/progress')
